...
export default function TemplatesPage() {
  ...
  // ── Category options (same pattern as TransactionForm) ──
  ...
  // ── Wallet selector ──
  const selectedAddWallet = wallets.find(w => w.id === addForm.walletId);
  const selectedEditWallet = wallets.find(w => w.id === editForm.walletId);
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    if (addForm.labelIds.length > 0) {
      await supabase.from('template_labels').insert(
        addForm.labelIds.map(lid => ({ template_id: inserted.id, label_id: lid }))
      );
    }
  ...
  async function handleEditSave(e: React.FormEvent) {
    ...
    if (editForm.labelIds.length > 0) {
      await supabase.from('template_labels').insert(
        editForm.labelIds.map(lid => ({ template_id: editingTemplate.id, label_id: lid }))
      );
    }
  ...
  async function handleDelete() {
    ...
    if (error) {
      setToast({ message: 'Failed to delete template.', variant: 'error' });
    } else {
  ...
  function toggleLabel(form: TemplateFormFields, id: string): TemplateFormFields {
    return {
      ...form,
      labelIds: form.labelIds.includes(id)
        ? form.labelIds.filter(l => l !== id)
        : [...form.labelIds, id],
    };
  }
  ...
  function renderForm(
    ...
    setForm: (f: TemplateFormFields) => void,
      ...
      <form onSubmit={onSubmit} className={styles.form}>
        ...
        <div className={styles.field}>
          ...
          <div className={styles.typeTabs}>
            <button
              type="button"
              className={[styles.typeTab, form.type === 'expense' ? styles.typeTabExpenseActive : ''].filter(Boolean).join(' ')}
            ...
            <button
              type="button"
              className={[styles.typeTab, form.type === 'income' ? styles.typeTabIncomeActive : ''].filter(Boolean).join(' ')}
              ...
              {labels.map(label => {
                ...
                return (
                  <button
                    ...
                    onClick={() => setForm(toggleLabel(form, label.id))}
                  >
                  ...
                  return (
                    <div key={tpl.id} className={styles.templateItem}>
                      <div
                        ...
                        style={{ backgroundColor: (tpl.category?.color ?? '#94a3b8') + '22' }}
                      >
                      ...
                      <div className={styles.templateInfo}>
                        ...
                        <div className={styles.templateMeta}>
                          <span className={[styles.typeBadge, tpl.type === 'expense' ? styles.typeBadgeExpense : styles.typeBadgeIncome].join(' ')}>
                            {tpl.type}
                          </span>
                          <span className={styles.templateAmount}>
                            {wallet ? formatCurrency(tpl.amount, wallet.currency) : String(tpl.amount)}
                          </span>
                          ...
                          {tpl.labels && tpl.labels.length > 0 && (
                            <span>· {tpl.labels.map(l => l.name).join(', ')}</span>
                          )}
                        </div>
                      </div>
